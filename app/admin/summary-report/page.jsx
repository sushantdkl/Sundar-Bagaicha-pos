    ...
    <main className="bg-gray-50 p-4 sm:p-6 lg:p-8 print:bg-white print:p-0">
      ...
      {loading&&!d?<div className="py-24 text-center text-sm text-gray-500">Building the report...</div>:d&&<div className="mx-auto max-w-7xl bg-white p-5 shadow-sm print:max-w-none print:p-0 print:shadow-none">
        ...
        <div className="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
          <Section title="Restaurant Revenue" note="Money the restaurant bills moved, by tender. The three lines above add to the total." rows={[line('Gross Cash Sales',d.revenue.cash),line('Gross Bank / Online Sales',d.revenue.bank),line('Gross Credit Sales',d.revenue.credit),line('Gross Restaurant Sales',d.revenue.restaurant)]}/>
          <Section title="Event Sales" note="Events use the same sales and customer-credit system as POS. Advances were collected earlier; cash/QR and credit below are the allocations made at completion." rows={[line('Recognised Revenue (net of tax)',d.events.net_revenue),line('Tax on Events',d.events.tax),line('Cash received at completion',d.events.cash),line('QR / bank received at completion',d.events.bank),line('Customer credit created',d.events.credit),line('Advances applied',d.events.advances_applied),line(`Gross Event Sales (${d.events.settlements||0} settled)`,d.events.gross)]}/>
          <Section title="Total Sales" note="Restaurant and events are the two channels of one business, so the headline is their sum. Net revenue subtracts refunds and voided bills." rows={[line('Restaurant',d.revenue.restaurant),line('Events',d.revenue.events),line('Gross Sales',d.revenue.gross),line(`Refunds (${d.revenue.refund_count||0})`,d.revenue.refunds,'-'),line(`Voided Bills (${d.revenue.void_count||0})`,d.revenue.voids,'-'),line('Net Revenue',d.revenue.net)]}/>
