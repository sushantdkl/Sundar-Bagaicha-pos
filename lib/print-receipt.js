...
export function receiptStyle(size = '80', { layout = 'kot' } = {}) {
  ...
  return `<style>
    /* @page carries margin only. The page SIZE is written at print time by
       openReceiptPrint(), which measures the rendered receipt and emits a
       valid two-length \`size\`. The obvious \`size: 80mm auto\` cannot be used:
       CSS Paged Media accepts \`<length>{1,2}\` or the bare keyword \`auto\`, so
       mixing the two makes the whole declaration invalid and Chromium drops
       it — silently falling back to A4 and feeding ~200mm of blank paper
       after every receipt. */
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Item table */
    table { width: 100%; table-layout: fixed; border-collapse: collapse; margin: ${bill ? '1mm 0 2mm' : '1mm 0'}; }
    ...
    /* The measured page height means there is normally nothing to break, but a
       driver that overrides it must not tear a total in half. Deliberately not
       applied to the item table — a long bill has to be free to flow. */
    .r-head, .r-doctype, .r-meta, .r-totals, .r-grand, .r-foot {
...
export function openReceiptPrint({ title = 'Receipt', size = '80', body = '', layout = 'kot' } = {}) {
  ...
  w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title>${receiptStyle(size, { layout })}</head><body>${body}
    <script>
      // Thermal paper is a continuous roll: the page must be exactly as tall as
      // the receipt. Anything taller feeds blank paper; anything shorter splits
      // the receipt across pages and strands the totals on a second sheet.
      // CSS alone cannot express "this paper width, content height", so the
      // height is measured here and written into a valid @page rule.
      function sizePage(){
        var b = document.body;
        // Measure the BODY, never documentElement: the root's scrollHeight is
        // floored at the viewport (the print window is 640px tall), which would
        // pad every short receipt back out to ~170mm of paper. The body is
        // height:auto, so its border box is the receipt and nothing else.
        var px = Math.ceil(b.getBoundingClientRect().height) || b.scrollHeight || 0;
        // CSS defines 1in = 96px = 25.4mm. +2mm of tail keeps the last line off
        // the cut edge and absorbs sub-pixel rounding.
        var mm = Math.max(20, Math.ceil(px * 25.4 / 96) + 2);
        var st = document.createElement('style');
        st.appendChild(document.createTextNode('@page { size: ${paper} ' + mm + 'mm; margin: 0; }'));
        document.head.appendChild(st);
      }
      var printed=false;
      window.onload=function(){ if(printed) return; printed=true; sizePage(); window.focus(); window.print();
