...
export async function pgRun(sql, params = [], client = null) {
  ...
  if (isInsert && !/\bRETURNING\b/i.test(statement)) {
    // RETURNING * rather than RETURNING id: not every table has an id column.
    // role_permissions is keyed on (role, permission_key), and asking it for
    // id raised 'column "id" does not exist', so saving the permission matrix
    // failed outright on Postgres. lastInsertRowid simply reads null for such
    // a table, which is what its callers already expect.
