...
import { ensureAccountingSchema, postJournal, currentDrawerId, accountBalance } from '@/lib/accounting.js';
...
export async function recordAdvance(db, data, givenBy = null) {
  ...
  return db.transaction(async (tx) => {
    ...
    const available = await accountBalance(tx, creditCode, drawerId ? { drawerId } : {});
    if (amount > positiveMoney(available) + 0.001) {
      const sourceLabel = method === 'cash' ? 'cash in the active drawer' : 'online/bank balance';
      throw Object.assign(new Error(`Not enough ${sourceLabel} for this salary advance (available Rs ${positiveMoney(available).toFixed(2)}).`), { status: 400 });
    }
...
export async function recordPayment(db, data, paidBy = null) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (grossAmount > 0) {
      ...
      if (amount > positiveMoney(available) + 0.001) {
        const sourceLabel = method === 'cash' ? 'cash in the active drawer' : 'online/bank balance';
        throw Object.assign(new Error(`Not enough ${sourceLabel} for this salary payment (available Rs ${positiveMoney(available).toFixed(2)}).`), { status: 400 });
      }
      await postJournal(tx, {
