...
/**
 * Event sales recognised in the window.
 *
 * Restaurant revenue is measured from the money a bill journal moved, which
 * works because a bill is paid the moment it is raised. An event is not: part
 * of it arrives weeks earlier as an advance into 2030, and only the balance
 * moves money on settlement day. Measuring an event the restaurant way would
 * report a Rs 100,000 wedding as the Rs 70,000 collected at the door.
 *
 * So events are measured at what was actually recognised — the credits the
 * settlement posted to revenue and tax — which is the gross figure comparable
 * to a bill's total. `collected` is reported alongside it precisely so the two
 * can be told apart: gross is the sale, collected is the cash it moved today.
 *
 * Source type 'event_sale' is written once per event by finaliseBilling and is
 * guarded against duplicates there, so this cannot count a sale twice.
 */
async function eventSales(db, from, to) {
  const row = await db.get(`SELECT
      COALESCE(SUM(CASE WHEN a.code='4010' THEN jl.credit-jl.debit ELSE 0 END),0) AS net_revenue,
      COALESCE(SUM(CASE WHEN a.code='2020' THEN jl.credit-jl.debit ELSE 0 END),0) AS tax,
      COALESCE(SUM(CASE WHEN a.code='2030' THEN jl.debit-jl.credit ELSE 0 END),0) AS advances_applied,
      COUNT(DISTINCT je.id) AS settlements
    FROM journal_lines jl JOIN journal_entries je ON je.id=jl.journal_id JOIN accounts a ON a.id=jl.account_id
    WHERE je.source_type='event_sale' AND je.entry_date BETWEEN ? AND ?`, [from, to])
    .catch(() => null);
  const net = round2(row?.net_revenue);
  const tax = round2(row?.tax);
  return {
    net_revenue: net,
    tax,
    gross: round2(net + tax),
    advances_applied: round2(row?.advances_applied),
...
export async function buildSummaryReport(db, { start, end }) {
  ...
  const [revenue, events, eventMedia, refundMedia, voidMedia, corrections, ledger, purchases, expenses, salaryRows, cashAccount, bankAccount, savings, saleCategories, purchaseCategories, quantities, settings, costMap] = await Promise.all([
    journalMedia(db,start,end,['bill','bill_supplement']),
    eventSales(db,start,end),
    journalMedia(db,start,end,['event_sale']),
  ...
  // Restaurant and events are the two sales channels of one business, so the
  // headline is their sum. `revenue` stays restaurant-only underneath it, which
  // is what makes the split reportable rather than just the total.
  const restaurantRevenue=round2(revenue.total);
  const grossRevenue=round2(restaurantRevenue+events.gross);
  events.cash=eventMedia.cash;
  events.bank=eventMedia.bank;
  events.credit=eventMedia.credit;
  events.collected=round2(eventMedia.cash+eventMedia.bank);
  const netRevenue=round2(grossRevenue-refunded-voided);
  const received={
    gross_cash:round2(revenue.cash+eventMedia.cash+ledger.cash),
    gross_bank:round2(revenue.bank+eventMedia.bank+ledger.bank),
  ...
  return { range:{start,end}, generated_at:new Date().toISOString(), restaurant_name:settings?.restaurant_name||'Restaurant', revenue:{...revenue,restaurant:restaurantRevenue,events:events.gross,gross:grossRevenue,refunds:refunded,voids:voided,net:netRevenue,refund_count:num(corrections,'refund_count'),void_count:num(corrections,'void_count')}, events, ledger, received, purchases, expenses, salary,
