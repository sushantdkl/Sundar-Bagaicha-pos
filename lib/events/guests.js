...
import { EVENT_AUDIT_ACTION, TERMINAL_STATUSES, isQuotationLocked } from './constants.js';
...
export async function updateGuestCounts(db, eventId, data = {}, actor = {}) {
  ...
  // Changing an agreed number is a decision, not a typo fix. A directly
  // confirmed booking remains in initial planning until its quotation is sent.
  const contracted = isQuotationLocked(event);
  ...
  if (contracted && changingGuarantee && !reason) {
    fail('A reason is required to change the guaranteed guest count after the quotation is agreed.', 400, {
...
export async function guestHistory(db, eventId, { limit = 50 } = {}) {
  ...
  return db.all(
    `SELECT * FROM event_audit
      ...
      LIMIT ${capped}`,
    [toId(eventId, 'event'), EVENT_AUDIT_ACTION.GUESTS_CHANGED, EVENT_AUDIT_ACTION.GUESTS_FINALIZED]
  );
}
