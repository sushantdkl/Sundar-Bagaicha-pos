...
async function writeEventBill(tx, {
  event, statement, payments, advanceApplied, collected, businessDayId, journalId, entryDate, actor,
}) {
  ...
  const settled = round2(advanceApplied + collected);
  const outstanding = round2(Math.max(0, grandTotal - settled));
  const status = outstanding <= 0.009 ? 'paid' : 'partially_paid';
  ...
  // Payment rows so the bill's media split matches the journal's. The advance
  // is one of them: it is money the guest genuinely paid for this bill, just
  // banked earlier, and omitting it would leave the bill looking short-paid.
  ...
  for (const p of payments) {
    await tx.run(
      ...
      [billId, p.amount, p.method, p.reference_number]
    );
  }
  ...
  void entryDate;
  return { bill_id: billId, bill_number: billNumber, order_id: orderId, status, outstanding };
}
...
export async function finaliseBilling(db, eventId, data = {}, actor = {}) {
  ...
  const journalId = await db.transaction(async (tx) => {
    ...
    billed = await writeEventBill(tx, {
      event, statement, payments, advanceApplied, collected,
      businessDayId, journalId: id, entryDate, actor,
    });
    ...
    await logEventAudit(tx, {
      ...
      next: {
        ...
        payments: payments.map((p) => `${p.method}:${p.amount}`),
      },
    });
    await logEventAudit(tx, {
