...
import { finalStatement } from './billing.js';
import {
  EVENT_STATUS,
...
export async function getEvent(db, id) {
  ...
  const event = await db.get(`${EVENT_SELECT} WHERE e.id = ?`, [toId(id, 'event')]);
...
/**
 * A new event inherits the restaurant's configured VAT and service charge.
 *
 * The columns default to 0 in the schema, which meant every event started at
 * 0% however the restaurant was configured — so a venue that charges 13% VAT
 * had to remember to set it by hand on each booking, and forgetting it
 * understates the tax liability on a six-figure wedding. The rates stay
 * editable per event through the charges endpoint; this only supplies the
 * starting value, and reads it from settings rather than hard-coding it.
 */
async function defaultEventCharges(db) {
  try {
    const rows = await db.all(
      `SELECT setting_key, setting_value FROM system_settings
        WHERE setting_key IN ('vat_percentage', 'service_charge_percentage')`
    );
    const map = Object.fromEntries((rows || []).map((r) => [r.setting_key, Number(r.setting_value)]));
    const clean = (v) => (Number.isFinite(v) && v >= 0 ? v : 0);
    return {
      tax_percent: clean(map.vat_percentage),
      service_charge_percent: clean(map.service_charge_percentage),
    };
  } catch {
    // Settings are optional; an event simply starts at zero without them.
    return { tax_percent: 0, service_charge_percent: 0 };
  }
}
...
export async function createEvent(db, data = {}, actor = {}) {
  ...
  const charges = await defaultEventCharges(db);
  ...
  const created = await db.transaction(async (tx) => {
    ...
    const result = await tx.run(
      ...
      [
        ...
        charges.tax_percent,
        charges.service_charge_percent,
        actor.id || null,
        actor.id || null,
      ]
    );
...
/**
 * Close an event that will never be billed — a hosted tasting, a goodwill
 * function, a staff party.
 *
 * Completion normally happens by settling (lib/events/billing.js), which is why
 * the status matrix has no route to COMPLETED. This is the documented exception
 * and it is deliberately awkward: it needs its own action, its own permission,
 * and a written reason, because an event closed this way books no revenue and
 * would otherwise look identical to one that was simply never invoiced.
 *
 * It refuses when money is actually involved: an event holding advances has a
 * liability to release and must go through settlement or cancellation instead.
 */
export async function completeWithoutCharge(db, id, reason, actor = {}) {
  const clean = String(reason || '').trim();
  if (!clean) fail('A reason is required to complete an event without billing it.');

  const existing = await getEvent(db, id);
  if (existing.status === EVENT_STATUS.COMPLETED) {
    fail('This event is already completed.', 409, { code: 'already_completed' });
  }
  if (!canBill(existing.status)) {
    fail(
      `A ${existing.status.toLowerCase()} event cannot be completed — confirm it first.`,
      409,
      { code: 'not_billable' }
    );
  }

  const held = await depositBalance(db, existing.id);
  if (held > 0.009) {
    fail(
      `${existing.event_number} is holding ${held} in advances. Settle the event so the advance is released, or cancel it and handle the refund.`,
      409,
      { code: 'advances_held', held }
    );
  }

  const statement = await finalStatement(db, existing.id);
  if (statement.grand_total > 0.009) {
    // A quotation with a value is a bill waiting to happen. Closing it unbilled
    // silently loses the sale, so the lines have to be zeroed (or marked
    // complimentary) before this route will accept the event.
    fail(
      `${existing.event_number} is quoted at ${statement.grand_total}. Bill it, or zero the quotation first if it is genuinely free of charge.`,
      409,
      { code: 'quote_has_value', grand_total: statement.grand_total }
    );
  }
  ...
  await db.transaction(async (tx) => {
    await tx.run(
      ...
      [EVENT_STATUS.COMPLETED, actor.id || null, existing.id]
    );
    await logEventAudit(tx, {
      action: EVENT_AUDIT_ACTION.COMPLETED,
      eventId: existing.id, entityType: 'event', entityId: existing.id, actor,
      reason: clean,
      previous: { status: existing.status },
      next: { status: EVENT_STATUS.COMPLETED, billed: false, no_charge: true },
    });
  });
