...
export async function collectDeposit(db, eventId, data = {}, actor = {}) {
  ...
  let depositId;
  try {
    depositId = await postDeposit(db, {
      event, amount, method, accountCode, idempotencyKey, businessDayId, receivedOn, data, actor,
    });
  } catch (error) {
    // Two submits of the same key can both pass the read above and race to the
    // insert; the unique index catches the loser. That is the idempotent case
    // arriving a moment late rather than a fault, so it returns the deposit
    // that won instead of a server error. Anything else is rethrown untouched.
...
export async function voidDeposit(db, depositId, reason, actor = {}) {
  ...
  const row = await db.get('SELECT * FROM event_deposits WHERE id = ?', [toId(depositId, 'deposit')]);
...
/** Postgres reports a duplicate as 23505; SQLite says so in the message. */
function isDuplicateKey(error) {
  return error?.code === '23505' || /unique constraint/i.test(String(error?.message || ''));
}

/** The insert, its journal and its audit row, in one transaction. */
async function postDeposit(db, { event, amount, method, accountCode, idempotencyKey, businessDayId, receivedOn, data, actor }) {
