...
export async function issueRevision(db, eventId, { audience = BEO_AUDIENCE.CUSTOMER, reason = null, final = false } = {}, actor = {}) {
  ...
  const event = await db.get('SELECT id, revision_number FROM events WHERE id = ?', [Number(eventId)]);
  const nextRevision = Number(event.revision_number || 1) + 1;

  await db.transaction(async (tx) => {
    await tx.run(
      'UPDATE events SET revision_number = ?, updated_by = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [nextRevision, actor.id || null, event.id]
    );
...
export async function revisionHistory(db, eventId, { limit = 25 } = {}) {
  ...
  const rows = await db.all(
    `SELECT id, actor_name, reason, previous_value, new_value, detail, created_at
      ...
      LIMIT ${capped}`,
    [Number(eventId), EVENT_AUDIT_ACTION.BEO_REVISED]
  );
  return rows.map((r) => {
    let snapshot = null;
