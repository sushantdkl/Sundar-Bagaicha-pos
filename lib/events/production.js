...
export async function startEvent(db, eventId, data = {}, actor = {}) {
  ...
  // Claim the event before anything moves.
  //
  // The two checks above compare a value that was read a moment ago, which is
  // no defence against simultaneous callers: three presses of Start arriving
  // together all read started_at as NULL, all pass, and all deduct. QA
  // reproduced exactly that — 60 kg consumed by a 20 kg event and three
  // fulfilment orders.
  //
  // This UPDATE is the guard instead. It is atomic on both engines, so of any
  // number of concurrent callers exactly one can move started_at away from
  // NULL; every other one matches zero rows and is refused before it touches
  // stock. The claim is released again if the work below fails, so a genuine
  // retry is still possible.
  const claim = await db.run(
    `UPDATE events SET started_at = CURRENT_TIMESTAMP, updated_by = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ? AND started_at IS NULL`,
    [actor.id || null, event.id]
  );
  if (!claim?.changes) {
    fail(`${event.event_number} is already being started.`, 409, { code: 'already_started' });
  }

  try {
    return await runProduction(db, event, data, actor);
  } catch (error) {
    // Nothing was committed, so the event must not be left looking started.
    await db.run('UPDATE events SET started_at = NULL WHERE id = ? AND status != ?',
      [event.id, EVENT_STATUS.IN_PROGRESS]).catch(() => {});
    throw error;
  }
}

/** The body of startEvent, run once the event has been claimed. */
async function runProduction(db, event, data = {}, actor = {}) {
  ...
  await db.transaction(async (tx) => {
    // started_at was set by the claim above; only the status moves here, so
    // the event reads as in progress only once production has really run.
    await tx.run(
      `UPDATE events
          SET status = ?, business_day_id = ?,
