...
export async function actualProfitability(db, eventId) {
  ...
  // Revenue, once the event has been settled, is what was actually recognised
  // as a sale — read from the settlement journal, which is the only
  // authoritative figure.
  //
  // It cannot be recomputed from events.total_amount after billing: settlement
  // rewrites that column to the grand total, which already contains the
  // additional sales and the tax. Adding the extras again double-counts them
  // (QA caught 102,000 reported as 117,260), and including VAT would treat a
  // tax liability as income.
  let revenue;
  let revenueBasis;
  const settlement = await db.get(
    `SELECT jl.credit AS amount
       FROM journal_entries je
       JOIN journal_lines jl ON jl.journal_id = je.id
       JOIN accounts a ON a.id = jl.account_id
      WHERE je.source_type = 'event_sale' AND je.source_id = ? AND a.code = '4010'`,
    [event.id]
  );
  if (settlement) {
    revenue = round2(settlement.amount);
    revenueBasis = 'recognised sales revenue from the settlement journal, net of tax';
  } else {
    revenue = round2(Number(event.total_amount || 0) + additionalSales);
    revenueBasis = 'provisional — the event has not been settled yet';
  }
