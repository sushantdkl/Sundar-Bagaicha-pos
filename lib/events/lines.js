...
import {
  ...
  EVENT_AUDIT_ACTION, isQuotationLocked, TERMINAL_STATUSES,
} from './constants.js';
...
export async function recalculateEventTotals(db, eventId, { tx = null } = {}) {
  const handle = tx || db;
  const event = await handle.get('SELECT * FROM events WHERE id = ?', [toId(eventId, 'event')]);
...
async function loadEditableEvent(db, eventId, { allowLocked = false } = {}) {
  ...
  if (!allowLocked && isQuotationLocked(event)) {
    fail(
...
async function buildLine(db, event, data, actor) {
  ...
  // Set only for a package priced by its tiers; see below.
  ...
  if (lineType === EVENT_LINE_TYPE.PACKAGE) {
    ...
    unitPrice = listPrice;
    // Keep the tier engine's own total. A progressive package blends several
    // tiers into a per-guest rate that rarely divides evenly — 100 guests at
    // 1200 plus 20 at 1000 is 1,166.666... a head — so multiplying the rounded
    // rate back out drifts away from the tier arithmetic. QA saw a 150,000
    // quotation billed as 150,000.40. The rate stays on the line for the
    // customer to read; the money comes from the engine.
    tieredTotal = round2(priced.total);
  ...
  // An explicit price or a giveaway replaces the tier arithmetic; otherwise the
  // engine's exact total wins over the rounded rate times the quantity.
  const lineTotal = (tieredTotal != null && !wantsOverride && !isComplimentary)
    ? tieredTotal
...
export async function updateLine(db, eventId, lineId, data = {}, actor = {}) {
  ...
  // An update holds the agreed per-guest rate rather than re-running the tier
  // table (see the snapshot rule at the top of this file), so the total here
  // is the agreed rate times the new quantity.
