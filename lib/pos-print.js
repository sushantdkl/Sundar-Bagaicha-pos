...
const RESTAURANT = 'Sundar Bagaicha Events';
 ...
 */
export function printKot(kot, { size = '80', reprint = false } = {}) {
  ...
  const body = `
    <div class="r-head">
      <div class="r-name">${esc(RESTAURANT)}</div>
...
function headerBlock(receipt) {
  const lines = [
    ...
    [receipt.restaurant_phone, receipt.pan_number ? `PAN: ${receipt.pan_number}` : (receipt.vat_number ? `VAT: ${receipt.vat_number}` : '')]
      .filter(Boolean).join(' · '),
  ].filter(Boolean);
  return `
    <div class="r-head">
      <div class="r-name">${esc((receipt.restaurant_name || RESTAURANT).toUpperCase())}</div>
