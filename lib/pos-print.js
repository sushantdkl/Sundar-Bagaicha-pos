...
export function printProforma(proforma, { size = '80', settings = {} } = {}) {
  ...
  const isEvent = order.order_type === 'event';
  const isTakeaway = !order.table_number && !isDelivery && !isEvent;

  const body = `
    ${headerBlock(resolveBusinessIdentity(settings))}
    ${doctypeBlock('PRE-BILL', ['PAYMENT PENDING'])}
    <div class="r-meta">
      ${metaRow(isTakeaway || isDelivery || isEvent ? 'Type' : 'Table', isEvent ? 'Event' : isDelivery ? 'Delivery' : isTakeaway ? 'Takeaway' : (order.table_number || '—'))}
      ${isEvent && order.event_number ? metaFull('Event', order.event_number) : (order.order_number ? metaFull('Order', compactOrderNumber(order.order_number)) : '')}
      ${order.customer_name ? metaFull('Customer', order.customer_name) : ''}
...
/**
 * The receipt markup for one settled bill, without opening a window.
 *
 * Split out from printFinalBill so a batch can put several bills into a single
 * print job — see printFinalBills below. One receipt, one template: a batch and
 * a single print can never drift apart.
 */
export function finalBillBody(receipt, { reprint = false } = {}) {
  ...
  const isEvent = receipt.order_type === 'event';
  const isTakeaway = !receipt.table_number && !isDelivery && !isEvent;
  ...
  const body = `
    ...
    <div class="r-meta">
      ...
      ${metaRow(isTakeaway || isDelivery || isEvent ? 'Type' : 'Table', isEvent ? 'Event' : isDelivery ? 'Delivery' : isTakeaway ? 'Takeaway' : receipt.table_number)}
      ${isEvent ? metaFull('Event', receipt.order_number) : metaFull('Order', compactOrderNumber(receipt.order_number))}
...
/** One settled bill, printed on its own. */
export function printFinalBill(receipt, { size = '80', reprint = false } = {}) {
  return openReceiptPrint({
    title: `Bill ${receipt.bill_number}`,
    size,
    body: finalBillBody(receipt, { reprint }),
    layout: 'bill',
  });
}

/**
 * Several bills in one print job, separated by a cut line.
 *
 * Thermal paper is a continuous roll, so a batch prints as one strip and the
 * operator tears at each marked line — which is how a roll is meant to be used
 * and avoids one print dialog per bill. On A4 it prints as a long narrow
 * column, which is fine for an office copy.
 *
 * Every copy is stamped DUPLICATE, because a batch is by definition a reprint
 * of bills that were already given to the customer.
 */
export function printFinalBills(receipts = [], { size = '80', reprint = true, title = 'Bills' } = {}) {
  const usable = (receipts || []).filter((r) => r && r.items?.length);
  if (!usable.length) return false;
  const cut = `<div style="text-align:center;font-size:10px;letter-spacing:2px;margin:6px 0 10px;">${'- '.repeat(16)}</div>`;
  const body = usable.map((r) => finalBillBody(r, { reprint })).join(cut);
  return openReceiptPrint({ title: `${title} (${usable.length})`, size, body, layout: 'bill' });
}
