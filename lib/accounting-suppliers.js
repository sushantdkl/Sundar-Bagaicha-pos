...
/**
 * Suppliers we owe, optionally narrowed to those active in a period.
 *
 * `outstanding` is always the balance as it stands now — a payable has no
 * meaningful "as at last Tuesday" on this screen. A range narrows *which
 * suppliers are listed* to those with AP movement in the window and reports
 * what moved, which is what the period chips on the ledger page are asking.
 *
 * Without a range this is byte-for-byte the original query, so the default
 * view of the Supplier Ledger is unchanged.
 */
export async function supplierPayables(db, { from = null, to = null } = {}) {
  ...
  const rows = await db.all(
    `SELECT s.id, s.name,
       COALESCE((SELECT SUM(jl2.credit - jl2.debit) FROM journal_lines jl2
                  WHERE jl2.supplier_id = s.id AND jl2.account_id = ?), 0) AS outstanding,
       COALESCE(SUM(jl.credit), 0) AS period_billed,
       COALESCE(SUM(jl.debit), 0) AS period_paid,
       COUNT(jl.id) AS period_entries
     FROM suppliers s
     JOIN journal_lines jl ON jl.supplier_id = s.id AND jl.account_id = ?
  ...
  return rows.map((r) => ({
    ...
    period_billed: round2(r.period_billed),
    period_paid: round2(r.period_paid),
