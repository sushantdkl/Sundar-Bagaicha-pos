...
/**
 * A second permission check inside a handler, for routes where one endpoint
 * carries several intents and the required permission is only known once the
 * body has been read — cancelling an event versus editing it, for example.
 *
 * The cache is already warm: requireAuth() loaded it for the route's own
 * permission, so this stays synchronous.
 *
 * @throws {Error} with status 403, which handleRouteError returns verbatim.
 */
