...
/**
 * Persist the shared Cash/QR/Credit shape for a bill without posting revenue.
 *
 * Event billing uses this after posting its own authoritative `event_sale`
 * journal. Keeping this part shared means event credit appears in the same
 * customer ledger and can be collected by the same Accounts Receivable flow,
 * without posting the sale twice.
 */
export async function recordSharedBillAllocations(db, {
  billId, billNumber, allocations, customer = null, actorId = null, requestKey,
  businessDayId = null,
}) {
  ...
  for (let index = 0; index < allocations.length; index += 1) {
    ...
    if (allocation.method !== 'credit') {
      const payment = await insertReceivedPayment(db, {
        billId, allocation, actorId, customerId, requestKey, index, businessDayId: dayId,
      });
      paymentId = payment.id;
      key = payment.key;
    }
    await insertAllocation(db, {
      billId, paymentId, allocation, actorId, customerId, key, businessDayId: dayId,
    });
    if (allocation.method === 'credit') {
      await db.run(
        ...
        [customerId, billId, allocation.amount, allocation.dueDate,
          allocation.notes || `Credit sale ${billNumber}`, actorId, `${key}:ledger`, dayId]
      );
    }
  }
  ...
  if (credit > 0) {
    await db.run(
      ...
      [fromCents(credit), customerId]
    );
  }
  const outstanding = fromCents(credit);
  const status = outstanding > 0 ? 'partially_paid' : 'paid';
  await db.run(
    `UPDATE bills SET customer_id=?, outstanding_amount=?, payment_status=?, status=?,
       paid_at=${outstanding > 0 ? 'NULL' : 'CURRENT_TIMESTAMP'} WHERE id=?`,
  ...
  return {
    ...
    allocations: allocations.map((row) => ({
      method: row.method, amount: row.amount, provider: row.provider,
      reference: row.reference, due_date: row.dueDate,
      cash_tendered: row.cashTendered, change: row.change || 0,
    })),
  };
}
...
export async function recordInitialSplitSettlement(db, {
  ...
  const dayId = businessDayId || await currentBusinessDayId(db, { required: true, allowStale: true });
  const settlement = await recordSharedBillAllocations(db, {
    billId, billNumber, allocations, customer, actorId, requestKey, businessDayId: dayId,
  });
...
export async function collectCreditBalance(db, {
  ...
  return db.transaction(async () => {
    ...
    const linkedEvent = await db.get(
      `SELECT o.event_id FROM orders o WHERE o.id=? AND o.event_id IS NOT NULL`,
      [bill.order_id]
    ).catch(() => null);
    ...
    for (let index = 0; index < allocations.length; index += 1) {
      ...
      if (linkedEvent?.event_id) {
        const day = await db.get('SELECT business_date FROM business_days WHERE id=?', [businessDayId]);
        await db.run(
          ...
          [linkedEvent.event_id, allocation.amount, allocation.method, allocation.reference,
            day?.business_date || null, allocation.notes || `Credit collection ${bill.bill_number}`,
            businessDayId, customer.id, actorId]
        );
      }
    }
    ...
    if (linkedEvent?.event_id) {
      await db.run(
        `UPDATE events SET outstanding_amount=?, payment_status=?,
            deposit_total=COALESCE(deposit_total,0)+?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
        [remaining, remaining > 0 ? 'PARTIALLY_PAID' : 'PAID', fromCents(requestedCents), linkedEvent.event_id]
      );
    }
...
export async function writeOffCreditBalance(db, {
  ...
  return db.transaction(async () => {
    ...
    if (linkedEvent?.event_id) {
      await db.run(
        `UPDATE events SET outstanding_amount=?, payment_status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
        [remaining, remaining > 0 ? 'PARTIALLY_PAID' : 'PAID', linkedEvent.event_id]
      );
    }
