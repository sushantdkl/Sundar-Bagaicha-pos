...
import { nepalDateString, nepalDateSql } from '@/lib/report-dates.js';
...
/**
 * Customers who owe money, optionally narrowed to those active in a period.
 *
 * `outstanding` is always the balance as it stands right now — that is what a
 * receivable means, and dating it would be wrong. The range narrows *which
 * customers are listed* to those with ledger movement in the window, and
 * reports what moved (`period_debit` / `period_credit`), which is the question
 * "This Week" on a ledger page is actually asking.
 *
 * With a range, a customer who settled up inside the window still appears —
 * their activity is the point — so the list is a period statement rather than
 * only a debtors list.
 */
export async function customerReceivables(db, { from = null, to = null } = {}) {
  ...
  const params = [];
  let dateWhere = '';
  if (from) { dateWhere += ` AND ${nepalDateSql('cl.created_at')} >= date(?)`; params.push(from); }
  if (to) { dateWhere += ` AND ${nepalDateSql('cl.created_at')} <= date(?)`; params.push(to); }

  const rows = await db.all(
    `SELECT c.id, c.name, c.phone, c.credit_limit, c.current_credit, c.is_vip, c.is_blacklisted,
            COALESCE(SUM(cl.debit), 0) AS period_debit,
            COALESCE(SUM(cl.credit), 0) AS period_credit,
            COUNT(cl.id) AS period_entries
       FROM customers c
       JOIN customer_ledger cl ON cl.customer_id = c.id
      WHERE 1 = 1 ${dateWhere}
      GROUP BY c.id, c.name, c.phone, c.credit_limit, c.current_credit, c.is_vip, c.is_blacklisted
      ORDER BY c.current_credit DESC, c.name ASC`,
    params
  );
  ...
  return (rows || []).map((r) => ({
    ...
    credit_limit: round2(r.credit_limit),
    outstanding: round2(r.current_credit),
    period_debit: round2(r.period_debit),
    period_credit: round2(r.period_credit),
    period_entries: Number(r.period_entries || 0),
    is_vip: Boolean(r.is_vip),
    is_blacklisted: Boolean(r.is_blacklisted),
  }));
}
