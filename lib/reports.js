 ...
 * Event Sales.
 *
 * The one rule this tab exists to hold: an event becomes a sale when it is
 * SETTLED, not when it is booked. Everything here is read from the settlement
 * journal that lib/events/billing.js posts once per event, so the figure shown
 * beside restaurant sales is money actually earned.
 *
 * Three numbers are kept apart on purpose, because adding them together is how
 * a banquet business ends up reporting revenue it has not made:
 *
 *   recognised   settled events — the only one that is a sale
 ...
 *
 * Restaurant sales are not recomputed here. They come from the same paid-bill
 * query the Sales tab uses, with event orders excluded, so the two tabs cannot
 * disagree with each other.
 */
async function eventSalesTab(db, range, f) {
  const cap = detailCap(f);
  const params = [range.start, range.end];
  const scope = billScope(range, f);

  // The events schema may not be installed on an older deployment. That is a
  // missing migration rather than a broken report, so it is said out loud.
  let installed = true;
  try {
    await db.get(`SELECT id FROM events WHERE 1 = 0`);
  } catch {
    installed = false;
  }
  if (!installed) {
    return {
      chips: [{ icon: 'info', text: 'The events module is not installed on this database.' }],
      kpis: [],
      charts: {},
      insights: [{
        title: 'Events schema missing',
        body: 'The events tables do not exist here. Run `npm run db:migrate` to apply migration 045 onwards, then reload this tab.',
        tone: 'warning',
      }],
      tables: [],
      notes: ['No event figures can be reported until the events migrations are applied.'],
    };
  }

  const [recognised, daily, byType, byPackage, restaurantRow, committed, deposits, settled, consumption, eventExpenses] =
    await Promise.all([
      db.get(
        `SELECT COALESCE(SUM(CASE WHEN a.code = '4010' THEN jl.credit ELSE 0 END), 0) AS net,
                COALESCE(SUM(CASE WHEN a.code = '2020' THEN jl.credit ELSE 0 END), 0) AS tax,
                COALESCE(SUM(CASE WHEN a.code = '2030' THEN jl.debit ELSE 0 END), 0) AS advances_applied,
                COUNT(DISTINCT je.source_id) AS events
           FROM journal_entries je
           JOIN journal_lines jl ON jl.journal_id = je.id
           JOIN accounts a ON a.id = jl.account_id
          WHERE je.source_type = 'event_sale' AND je.entry_date BETWEEN ? AND ?`,
        params
      ),
      db.all(
        `SELECT je.entry_date AS d,
                COALESCE(SUM(CASE WHEN a.code IN ('4010','2020') THEN jl.credit ELSE 0 END), 0) AS amount
           FROM journal_entries je
           JOIN journal_lines jl ON jl.journal_id = je.id
           JOIN accounts a ON a.id = jl.account_id
          WHERE je.source_type = 'event_sale' AND je.entry_date BETWEEN ? AND ?
      ...
      db.all(
        `SELECT COALESCE(NULLIF(TRIM(e.event_type), ''), 'Unspecified') AS event_type,
                COUNT(*) AS events,
                COALESCE(SUM(e.total_amount), 0) AS amount,
                COALESCE(SUM(COALESCE(e.actual_guests, e.guaranteed_guests, e.expected_guests, 0)), 0) AS guests
           FROM events e
          WHERE e.status = 'COMPLETED' AND e.event_date BETWEEN ? AND ?
          GROUP BY COALESCE(NULLIF(TRIM(e.event_type), ''), 'Unspecified')
          ORDER BY amount DESC`,
        params
      ),
      db.all(
        `SELECT COALESCE(p.name, 'Custom menu') AS package_name,
                COUNT(*) AS times_sold,
          ...
          WHERE l.line_type = 'package' AND e.status = 'COMPLETED'
            AND e.event_date BETWEEN ? AND ?
          GROUP BY COALESCE(p.name, 'Custom menu')
          ORDER BY amount DESC`,
        params
      ),
      db.get(
        `SELECT COALESCE(SUM(b.grand_total), 0) AS total, COUNT(DISTINCT b.id) AS bills
         ${BILL_FROM}${scope.sql} AND o.event_id IS NULL`,
        scope.params
      ),
      db.get(
        `SELECT COUNT(*) AS events, COALESCE(SUM(total_amount), 0) AS value
           FROM events
          WHERE status IN ('CONFIRMED','PLANNING','FINALIZED','IN_PROGRESS')
            AND event_date >= ?`,
        [nepalDateString()]
      ),
      db.get(
        `SELECT COALESCE(SUM(CASE WHEN d.entry_type = 'refund' THEN -d.amount ELSE d.amount END), 0) AS held
           FROM event_deposits d
           JOIN events e ON e.id = d.event_id
          WHERE d.status = 'active' AND e.status <> 'COMPLETED'`
      ),
      db.all(
        `SELECT e.id, e.event_number, e.title, e.event_type, e.event_date, e.status,
                ...
                e.total_amount, e.deposit_total, e.outstanding_amount,
                COALESCE((
      ...
      db.all(
        `SELECT o.event_id,
                COALESCE(SUM(-sm.quantity_changed * i.cost_per_unit), 0) AS food_cost
           FROM stock_movements sm
           JOIN inventory_items i ON i.id = sm.inventory_item_id
           JOIN orders o ON CAST(o.id AS TEXT) = sm.reference_id
           JOIN events e ON e.id = o.event_id
          WHERE sm.change_type = 'order_deduction' AND e.event_date BETWEEN ? AND ?
          GROUP BY o.event_id`,
        params
      ),
      db.all(
        `SELECT x.event_id, COALESCE(SUM(x.amount), 0) AS amount
           FROM expenses x
  ...
  const netRevenue = num(recognised, 'net');
  const tax = num(recognised, 'tax');
  const eventSales = netRevenue + tax;
  const eventCount = num(recognised, 'events');
  const restaurantSales = num(restaurantRow, 'total');
  const totalSales = restaurantSales + eventSales;

  const foodCostByEvent = new Map((consumption || []).map((r) => [r.event_id, num(r, 'food_cost')]));
  const expenseByEvent = new Map((eventExpenses || []).map((r) => [r.event_id, num(r, 'amount')]));
  const settledCapped = capRows(settled, cap);

  const eventRows = settledCapped.rows.map((r) => {
    const revenue = num(r, 'recognised');
    const food = foodCostByEvent.get(r.id) || 0;
    const spend = expenseByEvent.get(r.id) || 0;
    const contributionRow = revenue - food - spend;
    return {
      event_number: r.event_number,
      title: r.title || '—',
      event_type: r.event_type || '—',
      event_date: dateKey(r.event_date),
      guests: num(r, 'guests'),
      revenue,
      food_cost: food,
      event_expenses: spend,
      contribution: contributionRow,
      margin: revenue > 0 ? (contributionRow / revenue) * 100 : 0,
    };
  });

  const totalGuests = eventRows.reduce((s, r) => s + r.guests, 0);
  const totalFood = eventRows.reduce((s, r) => s + r.food_cost, 0);
  const totalSpend = eventRows.reduce((s, r) => s + r.event_expenses, 0);
  const contribution = netRevenue - totalFood - totalSpend;

  const chips = [];
  chips.push({ icon: 'wallet', text: `${money(eventSales)} of event sales from ${eventCount} settled event(s)` });
  if (totalSales > 0) {
    chips.push({ icon: 'info', text: `Events are ${Math.round((eventSales / totalSales) * 100)}% of the ${money(totalSales)} the business took` });
  }
  if (num(committed, 'value') > 0) {
    chips.push({ icon: 'star', text: `${money(num(committed, 'value'))} is contracted for ${num(committed, 'events')} event(s) still to come — not counted as sales` });
  }
  if (num(deposits, 'held') > 0) {
    chips.push({ icon: 'info', text: `${money(num(deposits, 'held'))} of advances is held and still owed back` });
  }

  const insights = [];
  insights.push({
    title: 'What counts as an event sale',
    body: 'An event is a sale on the day it is settled, not the day it is booked. Deposits sit in Event Customer Advances until then, and a quotation for a future event is contracted value, not revenue. Only the settled figure is added to restaurant sales.',
    tone: 'neutral',
  });
  if (netRevenue > 0 && totalFood > 0) {
    insights.push({
      title: 'Food cost on events',
      body: `${money(totalFood)} of raw material was consumed against ${money(netRevenue)} of event revenue — ${Math.round((totalFood / netRevenue) * 100)}%. This comes from the stock ledger, so it reflects what the kitchen actually used rather than what the quotation assumed.`,
      tone: totalFood / netRevenue > 0.45 ? 'warning' : 'positive',
    });
  } else if (netRevenue > 0) {
    insights.push({
      title: 'No food cost recorded',
      body: 'No raw material was deducted against these events, so contribution here is revenue less booked expenses only. Enter recipe ingredients for the package menus to see a real food cost.',
      tone: 'warning',
    });
  }
  if (byType?.[0] && eventCount > 1) {
    insights.push({
      title: 'Event mix',
      body: `${byType[0].event_type} is the largest category at ${money(num(byType[0], 'amount'))} across ${num(byType[0], 'events')} event(s).`,
      tone: 'neutral',
    });
  }
  ...
  return {
    chips,
    kpis: [
      { key: 'event_sales', label: 'Event Sales', value: eventSales, format: 'currency', sub: `${eventCount} settled · incl. tax` },
      { key: 'restaurant_sales', label: 'Restaurant Sales', value: restaurantSales, format: 'currency', sub: `${num(restaurantRow, 'bills')} bills` },
      { key: 'total_sales', label: 'Total Sales', value: totalSales, format: 'currency', sub: 'Restaurant + events, counted once' },
      { key: 'committed', label: 'Contracted, Not Earned', value: num(committed, 'value'), format: 'currency', sub: `${num(committed, 'events')} upcoming` },
      { key: 'deposits_held', label: 'Advances Held', value: num(deposits, 'held'), format: 'currency', sub: 'A liability, not income' },
      { key: 'contribution', label: 'Event Contribution', value: contribution, format: 'currency', sub: 'Revenue less food and event expenses' },
      { key: 'per_event', label: 'Average Per Event', value: eventCount ? eventSales / eventCount : 0, format: 'currency' },
      { key: 'per_guest', label: 'Average Per Guest', value: totalGuests ? netRevenue / totalGuests : 0, format: 'currency', sub: `${totalGuests} guests served` },
    ],
    charts: {
      overTime: days.map((d) => ({ label: d.day, sub: d.date, value: dailyMap.get(d.date) || 0 })),
      channelSplit: [
        { label: 'Restaurant', value: restaurantSales },
        { label: 'Events', value: eventSales },
      ],
      byType: (byType || []).map((r) => ({ label: r.event_type, value: num(r, 'amount'), meta: `${num(r, 'events')} event(s)` })),
      byPackage: (byPackage || []).map((r) => ({ label: r.package_name, value: num(r, 'amount'), meta: `${num(r, 'guests')} guests` })),
    },
    insights,
    tables: [
      {
        id: 'settled-events',
        title: 'Settled Events',
        truncated: settledCapped.truncated,
        limit: cap,
        columns: [
          ...
          { key: 'guests', label: 'Guests', type: 'number', align: 'right' },
          { key: 'revenue', label: 'Revenue', type: 'currency', align: 'right' },
          { key: 'food_cost', label: 'Food Cost', type: 'currency', align: 'right' },
          { key: 'event_expenses', label: 'Expenses', type: 'currency', align: 'right' },
          { key: 'contribution', label: 'Contribution', type: 'currency', align: 'right' },
          { key: 'margin', label: 'Margin', type: 'percent', align: 'right' },
        ],
        rows: eventRows,
        empty: 'No event was settled in this period.',
      },
      {
        id: 'event-types',
        title: 'By Event Type',
        columns: [
          ...
          { key: 'events', label: 'Events', type: 'number', align: 'right' },
          { key: 'guests', label: 'Guests', type: 'number', align: 'right' },
          { key: 'amount', label: 'Contracted Value', type: 'currency', align: 'right' },
        ],
        rows: (byType || []).map((r) => ({
          event_type: r.event_type,
          events: num(r, 'events'),
        ...
        empty: 'No completed events in this period.',
      },
      {
        id: 'event-packages',
        title: 'Package Mix',
        columns: [
          ...
          { key: 'times_sold', label: 'Times Sold', type: 'number', align: 'right' },
          { key: 'guests', label: 'Guests', type: 'number', align: 'right' },
          { key: 'amount', label: 'Value', type: 'currency', align: 'right' },
        ],
        rows: (byPackage || []).map((r) => ({
          package_name: r.package_name,
          times_sold: num(r, 'times_sold'),
          guests: num(r, 'guests'),
          amount: num(r, 'amount'),
        })),
        empty: 'No package was sold on a completed event in this period.',
      },
    ],
    notes: [
      'Event Sales is the gross figure — revenue plus tax — so it is directly comparable to a restaurant bill total. Restaurant Sales excludes every order raised by an event, so the two never overlap.',
      'Contracted value and advances held are shown for cash planning and are deliberately excluded from every sales figure on this page.',
      'Food cost is read from the stock ledger for orders raised by each event. It reads zero for any package whose menu items have no recipe ingredients.',
    ],
  };
}
