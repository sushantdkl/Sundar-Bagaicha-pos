...
async function repairMissingSaleJournals(db, dayId) {
  ...
  // Back-post a sale journal for any settled bill that never got one.
  //
  // "Never got one" has to mean more than "has no source_type='bill' entry".
  // An event bill is posted by lib/events/billing.js under source_type
  // 'event_sale' and names that entry in bills.journal_id — it is fully
  // journalled, just not by this path. Repairing it would post a SECOND
  // revenue entry and double that event's sale, which is the one outcome the
  // whole events design exists to prevent. Both guards below exclude it:
  // journal_id is the general rule, the event_id check covers any bill written
  // before journal_id existed.
  const bills = await safe(db.all(`
    ...
    WHERE b.business_day_id = ? AND ${settled} AND LOWER(COALESCE(b.status,'')) <> 'voided'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM orders o WHERE o.id = b.order_id AND o.event_id IS NOT NULL
      )
      AND (
  ...
  for (const bill of bills || []) {
    ...
    await postSaleJournal(db, {
      ...
      // businessDateKey, not String(...).slice(0, 10): Postgres returns a Date
      // object here, whose toString is "Thu Aug 20 2026 …" — slicing that
      // yields "Thu Aug 20" and the INSERT dies on an invalid date, taking the
      // whole business-day open down with it.
      entry_date: businessDateKey(bill.paid_at || bill.created_at) || null,
      parts,
...
export async function businessDaySummary(db, dayOrId) {
  ...
  const [sales, channels, paymentRows, corrections, expenses, orders, kots, itemTotals, topItems, tables, cashRows, onlineRows, blockers] = await Promise.all([
    ...
    safe(db.all(`SELECT CASE WHEN o.event_id IS NOT NULL THEN 'event'
                        ELSE LOWER(COALESCE(o.order_type,'other')) END AS channel,
      ...
      AND LOWER(COALESCE(b.status,''))<>'voided'
      GROUP BY CASE WHEN o.event_id IS NOT NULL THEN 'event'
                    ELSE LOWER(COALESCE(o.order_type,'other')) END
