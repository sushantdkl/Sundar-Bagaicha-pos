...
export async function createSavingsDeposit(db, data, actorId) {
  ...
  return db.transaction(async (tx) => {
    ...
    const available = await accountBalance(tx, sourceCode, drawerId ? { drawerId } : {});
    if (amount > round2(available) + 0.001) fail(`Not enough ${source === 'cash' ? 'cash in the active drawer' : 'online/bank balance'} (available Rs ${round2(available).toFixed(2)}).`);
...
export async function voidSavingsDeposit(db, id, reason, actorId) {
  ...
  return db.transaction(async (tx) => {
    const originalCashLine = row.source_account === 'cash' && row.journal_id
      ? await tx.get(`SELECT jl.drawer_id FROM journal_lines jl
          JOIN accounts a ON a.id=jl.account_id
          WHERE jl.journal_id=? AND a.code='1010' ORDER BY jl.id LIMIT 1`, [row.journal_id])
      : null;
    const drawerId = row.source_account === 'cash'
      ? (originalCashLine?.drawer_id || await currentDrawerId(tx))
      : null;
