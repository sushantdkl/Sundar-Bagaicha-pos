            ...
            <div className="rounded-xl border border-sky-200 bg-sky-50/70 p-3">
              ...
              {deliveryEnabled && (
                <div className="mt-3 space-y-3">
                  ...
                  {/* Assignment stays optional so checkout is never blocked,
                      but the control stays visible when the list is empty. */}
                  <label className="block text-xs font-bold text-slate-900">
                    Delivery executive
                    <select
                      value={deliveryExecutiveId || ''}
                      onChange={(e) => onDeliveryExecutiveChange?.(e.target.value)}
                      disabled={deliveryExecutives.length === 0}
                      className="mt-1 w-full rounded-lg border-2 border-sky-200 bg-white px-3 py-2 text-sm font-bold text-slate-900 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100 disabled:cursor-not-allowed disabled:bg-sky-100/60 disabled:text-sky-800"
                    >
                      <option value="">
                        {deliveryExecutives.length ? 'Assign later' : 'No delivery executives available'}
                      </option>
